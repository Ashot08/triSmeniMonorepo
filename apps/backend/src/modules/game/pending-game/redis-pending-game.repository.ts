...
export class RedisPendingGameRepository extends PendingGameRepository {
  ...
  async findByOrganization(organizationId: string): Promise<PendingGame[]> {
    const ids = await this.redis.sMembers(this.organizationKey(organizationId));

    return this.loadGames(ids);
  }

  async findPublic(): Promise<PendingGame[]> {
    const ids = await this.redis.sMembers(this.publicGamesKey());
  ...
  async create(game: PendingGame): Promise<void> {
    ...
    if (existing) {
      throw new ConflictException('Pending game already exists.');
    }
  ...
  async update(game: PendingGame): Promise<void> {
    ...
    try {
      ...
      if (!stored) {
        throw new NotFoundException('Pending game not found.');
      }

      if (stored && stored.getVersion() !== game.getVersion()) {
        throw new ConflictException(
          'Pending game was modified by another request.',
        );
      }
  ...
  private async loadGames(ids: string[]): Promise<PendingGame[]> {
    const games = await Promise.all(ids.map((id) => this.findById(id)));

    return games.filter((game): game is PendingGame => game !== null);
  }
